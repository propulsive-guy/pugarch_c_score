...
if uploaded_file:
    ...
    # Load YOLO model (relative path for GCP compatibility)
    ...
    # Raw weight per class (based on dirtiness severity)
    original_weights = {
        0: 1,
        1: 5,
        2: 4,
    ...
    # Normalize weights so total sums to 10
    total_weight = sum(original_weights.values())
    normalized_weights = {cls_id: (weight / total_weight * 10) for cls_id, weight in original_weights.items()}

    # Compute cleanliness score with confidence consideration
    raw_score = 0
    ...
    st.markdown("###  Class-wise Detection (Confidence-Weighted)")
    for cls_id, conf_list in class_confidence_dict.items():
        count = len(conf_list)
        avg_conf = np.mean(conf_list)
        st.write(f"- **{class_mapping[cls_id]}**: {count} × avg confidence {round(avg_conf, 2)} × weight {round(normalized_weights[cls_id], 2)}")

    # Display scores
    st.markdown(f"###  Raw Cleanliness Score (higher = dirtier): **{round(raw_score, 2)}**")
    st.markdown(f"###  Final Cleanliness Score (out of 10): **{cleanliness_score}**")

    # Generate and display detection image
    with tempfile.TemporaryDirectory() as pred_dir:
        model.predict(image_path, save=True, save_txt=False, save_conf=True, project=pred_dir, name="result", exist_ok=True)
